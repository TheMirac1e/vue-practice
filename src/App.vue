    ...
    <div class="container">
      ...
      <div v-if="tickers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div class="bg-white overflow-hidden shadow rounded-lg border-purple-800 border-solid cursor-pointer" :class="{
            'border-4': sel === item
          }" v-for="item in tickers" @click="sel = item" :key="item.name">
      ...
      <section class="relative" v-if="sel">
        ...
        <button type="button" class="absolute top-0 right-0" @click="select(t)">
...
<script>
import { API_KEY } from './static/static.js'
  ...
  methods: {
    subscribeToUpdates (tickkerName) {
      setInterval(async () => {
        ...
        this.tickers.find((t) => t.name === tickkerName).price =
          data.USD > 1 ? data.USD.toFixed(2) : data.USD.toPrecision(2)

        if (this.sel?.name === tickkerName) {
